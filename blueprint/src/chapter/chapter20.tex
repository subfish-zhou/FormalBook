...
\begin{proof}
  \uses{ch20theorem2}
  P\'olya's proof via symmetrization and AM-GM. We write
  $f(x) = (1-x^2)\prod(\alpha_i - x)\prod(\beta_j + x)$ and show:
  (1) $-f'(1)f'(-1) = 4C^2$ where $C^2 = \prod(\alpha_i^2-1)\prod(\beta_j^2-1)$;
  (2) $T \le \sqrt{-f'(1)f'(-1)}$ by HM-GM;
  (3) $A \ge (4/3)C$ by symmetrization + AM-GM on the integral.
  Combining gives $A \ge (2/3)T$.
  Algebraic identities fully formalized; integral layer uses sorry.
\end{proof}
