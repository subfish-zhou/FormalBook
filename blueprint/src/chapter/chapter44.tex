...
\textbf{Lean formalization note:} Step~(1) — the proof that $G$ is regular, that
$n = k^2 - k + 1$, and that $A^2 = (k-1)I + J$ — is delegated to Mathlib's
\texttt{Archive.Wiedijk100Theorems.FriendshipGraphs} module, which provides
\texttt{Friendship.isRegularOf\_not\_existsPolitician},
\texttt{Friendship.card\_of\_regular}, and
\texttt{Friendship.adjMatrix\_sq\_of\_regular}.
...
\textbf{Lean formalization note:} Rather than analyzing the eigenvalues of $J$ first and then
deducing those of $A^2$, the Lean proof derives the eigenvalues of $A^2$ directly via the
annihilating polynomial $(A^2 - (k-1)I)(A^2 - k^2 I) = 0$, which follows from $A^2 = (k-1)I + J$
and $J^2 = nJ$. This shows every eigenvalue of $A^2$ lies in $\{k-1, k^2\}$.
...
\textbf{Lean formalization note:} The Lean proof uses an equivalent algebraic form of this
lemma: if $p^2 = m \cdot q^2$ with $\gcd(p,q) = 1$, then $q = 1$
(\texttt{Nat.sqrt\_rational\_is\_integer}). This follows from coprimality and divisibility
rather than the minimal-element argument above.
