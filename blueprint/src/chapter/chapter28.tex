...
\begin{theorem}[Pigeon-hole principle]
  ...
  \lean{chapter28.pigeon\_hole\_principle}
...
\begin{theorem}[Claim]
  \label{ch28claim1}
  \lean{chapter28.claim1\_coprime}
...
\begin{theorem}[Claim]
  \label{ch28claim2}
  \lean{chapter28.claim2\_divisible}
...
\begin{theorem}[Claim (Erd\H{o}s--Szekeres)]
  \label{ch28claim3}
  \lean{chapter28.claim3\_erdos\_szekeres}
...
\begin{theorem}[Claim]
  \label{ch28claim4}
  \lean{chapter28.claim4\_contiguous\_sum}
...
\begin{theorem}[Double counting]
  ...
  \lean{chapter28.double\_counting}
...
\begin{theorem}[Average number of divisors]
  ...
  \lean{chapter28.sum\_divisor\_count}
...
\begin{theorem}[Reiman]
  \label{ch28theorem}
  \lean{chapter28.c4\_free\_edge\_bound}
...
\begin{theorem}[Cherry counting]
  ...
  \lean{chapter28.sum\_choose\_deg\_le\_choose\_card}
  \leanok
  If the graph $G$ on $n$ vertices contains no $4$-cycles, then
  \[
...
\begin{theorem}[Brouwer's Fixed Point Theorem (for $n = 2$)]
  \label{brouwer}
  \lean{chapter28.brouwer\_fixed\_point\_2d}
